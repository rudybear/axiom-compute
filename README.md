...
- **Multi-tile cooperative_matrix matmul (M3.3b)**: a real tiled tensor-core matmul — a grid of workgroups each computing a 16×16 output tile, K-looping with the OpPhi accumulator — is **bit-exact on NVIDIA** (full M×N), at a first honestly-measured **5.0 TFLOPS (≈4% of the cuBLAS f32 datasheet estimate)** from one annotated `.axc` source → portable coopmat SPIR-V. (Single-subgroup-per-tile; register/multi-warp blocking + non-multiple-of-16 tiles are follow-ups.)
