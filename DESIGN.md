...
**AT-2105 — honest latency, no beat-cuBLAS claim.** Three CUDA-event min-of-N numbers at the A/B shape: (a) AXIOM zero-copy `matmul()` (dispatch+handshake, x already resident), (b) AXIOM host-copy baseline (CPU→.cuda()→shared x→matmul→C→.cpu(), the host round-trip zero-copy ELIMINATES), (c) native `torch.matmul` (cuBLAS, context only). The ONLY hard latency gate is **(a) < (b)** (the no-host-transfer win); (c) is REPORTED, NOT gated. Stated plainly: the Vulkan kernel is ~34% of cuBLAS (M3.6 42.86 TFLOPS; cuBLAS faster still), so end-to-end COMPUTE is SLOWER than native torch — the value is no-host-copy + real torch interop, not beating cuBLAS (R-6).
