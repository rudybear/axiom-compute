...
| **llama.cpp Vulkan head-to-head A/B** | NVIDIA done (M3.4 matvec; M3.5 fused SAME-SHAPE). M3.4: AXIOM single-row matvec ≈ 87,000× below llama Q4_K n=1 GEMV (cross-shape baseline). M3.5: fused Q4_K_M dequant→RB coopmat GEMM, re-run SAME-SHAPE (AXIOM **11.27 TFLOPS** vs llama Q4_K MUL_MAT @ m=4096,n=512,k=14336 = **101.48 TFLOPS**) → **~9× behind on throughput** (down from ~87,000× cross-shape). **BUT NOT a usable-kernel win: AXIOM is NUMERICALLY INVALID at k=14336 (max-rel-diff 29.07 — f16 accumulator overflows at inference K; correct only at K≤256). Fast-but-WRONG; needs f32-accumulator coopmat (M3.5b).** Kill-criterion FAIL on NVIDIA (behind AND invalid); AMD/Intel pending hw |
| **Q4_K_M coopmat fusion (M3.5)** | ✅ landed — `f32_to_f16` builtin (no new capability beyond M3.3c∪M2.6 union) + fused `q4km_matmul_rb_coopmat.axc`. **CORRECTNESS IS K-LIMITED**: within FROZEN 1e-3 vs f16-accumulator ggml ref only at K≤256 (AT-1770 K=256 PASS 8.3e-4; AT-1771 K=512 EXCEEDS 1e-3, gate capped, 1e-3 NOT loosened); at k=14336 max-rel-diff=29.07 (garbage). Throughput SAME-SHAPE A/B ~9× behind but fast-but-WRONG at inference K — M3.5b needs f32-accumulator coopmat (`--fused` → ab_results_fused.json) |
