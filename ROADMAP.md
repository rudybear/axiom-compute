...
| M3.22 PW-1 (LOW): a @strategy hole used ONLY in a shared[T,?WG] length (not in any annotation) is spuriously reported UnusedStrategyHole on the raw lower path — reference-tracking scans annotations only; one-line fix (add shared-decl len_hole names to referenced_holes) | low | 20 LOC |
