...
| **Memory-subsystem levers (M3.10)** | 🟡 PROPOSED (Architect, `.pipeline/runs/M310-architect.json`, DESIGN.md §3.1.37) — the campaign's second compiler-feature milestone, targeting the bottleneck M3.7/M3.8/M3.9 isolated by elimination (NOT latency, NOT arithmetic-intensity, NOT warp-occupancy, NOT staging-volume → the **memory subsystem**). **Codegen-feasibility resolved:** (1) **bank padding (`SHMEM_STRIDE`) is PURE SOURCE** — `coopmat_load` already takes a runtime `u32` stride (coopmat.rs:269/323) and shared access is a single-index runtime `u32` (shared.rs:177/202), so a padded shared tile `shared[f16, rows*(cols+PAD)]` + padded write index + padded coopmat_load stride needs ZERO codegen change; (2) **vectorized loads (`LOAD_VEC`) are NOT pure source** — `OpMemoryModel Logical Vulkan` (emit.rs:159, no PhysicalStorageBuffer) forbids ptr-to-vec bitcast, so a real vec4 `OpLoad` needs a vec-typed SSBO **view** (`OpTypeRuntimeArray %v4f16`, ArrayStride=8 = a layout change) AND AXIOM has **no vector types** in the type system (ty.rs) → a genuine codegen+layout lift, not a minimal builtin. **SCOPE: M3.10a = pure-source bank padding (ship), plain-f32 RB first then conditionally the M3.6 Q4_K_M leader; M3.10b = a conditional/DEFERRABLE vec-load feasibility spike** (`load_vec4_to_shared` self-contained scatter, gated behind M3.10a's signal). Per-feature ≥1.15× gate vs the relevant leader + armed honest-negative (the M3.7/M3.8/M3.9/M3.3d occupancy/memory quartet makes a clean win UNCERTAIN — measured ratio + bank/occupancy diagnosis reported either way, gate never loosened). Bit-identical (plain) / combined-VALID (Q4_K_M) on the asymmetric fixture; FROZEN 1e-3 untouched; spirv-val + real-NVIDIA mandatory; vec-load AT carries an emitted-vector-`OpLoad` falsifier. AT-2400..2412. |
