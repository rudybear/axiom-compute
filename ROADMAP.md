...
| Cross-vendor real GPU CI (AMD RDNA3+, Intel Arc) | ✅ EB.1-prep MERGED (workflows + docs authored, `AMD_RUNNER_ONLINE`/`INTEL_RUNNER_ONLINE` activation ready) — ⛔ measurement remains HARDWARE-GATED (EB.1 proper): this box has only NVIDIA. The SINGLE remaining blocker for BOTH the project's cross-vendor kill-criterion ("beat by ≥25% where llama.cpp Vulkan is weak") AND a compelling upstream PR (M4.2). EB.2 (per-machine baselines, MERGED) made the bench-regression harness cross-vendor-ready (each machine keyed by sanitized deviceName, blessed independently). EB.1-prep delivers `cross-vendor-ci.yml` (per-PR AMD/Intel GPU-test + bench-regression gates, `if:`-gated on the activation variable + a same-repo fork guard — never `runs-on:`-gated, avoiding the queue-forever trap), `cross-vendor-ab.yml` (`workflow_dispatch`-only llama.cpp A/B evidence job for the RFC trigger), `cross-vendor-bless.yml` (`workflow_dispatch`-only first-run baseline bless, artifact→human-PR, never a runner push), hardens the previously-LIVE fork-PR hole in `ci.yml`'s self-hosted `pytorch-interop` job (same-repo guard + `permissions: contents: read` + `timeout-minutes: 30`), and ships `docs/EB1_RUNNER_SETUP.md` + a 17-AT CPU-only `saphyr`-backed structural/security test suite (`crates/axc-driver/tests/eb1_workflow_ci.rs`, AT-3019..3035) that scans every self-hosted job across every workflow file and fails closed on any non-literal `runs-on:` or missing fork-guard/permissions/timeout. M4.2 upstream RFC stays PREPARED-AND-HELD (`UPSTREAM_PR_PLAN.md`): integration into llama.cpp Vulkan is feasible-but-not-drop-in, but AXIOM is 2.39× behind on NVIDIA (architectural) and the AMD/Intel value prop is unmeasured → don't submit until an AMD/Intel A/B shows parity-or-better from one unchanged source, now a single `workflow_dispatch` click away once a runner registers |
...
| **Q6_K + Q5_K_M quantization variants (M3.14, FG.5)** | ✅ MERGED (2026-07-13) — COVERAGE milestone (no TFLOPS gate; M3.13 closed the perf campaign): real "Q4_K_M" GGUFs store `output`/`attn_v` tensors as **Q6_K**, so this closes an actual beachhead hole. **ZERO compiler changes** (signed int8 scales + the (q−32) bias handled pure-source: `f32_from_u32(u)` then conditional −256.0 — proven bit-exact-safe for all byte values). Deliverables: `q6k_oracle.rs`/`q5k_oracle.rs` CPU oracles (byte-exact vs ggml layouts: block_q6_K 210 B ql[128]/qh[64]/int8 scales[16]/d-LAST-no-dmin; block_q5_K 176 B d/dmin/12 B packed get_scale_min_k4/qh[32]/qs[128]), portable `q6k_dequant_matmul.axc` + `q5k_dequant_matmul.axc`, and `q5km_matmul_rb_coopmat_f32acc_cached.axc` = the M3.6 leader VERBATIM + 4 documented deltas (stride 144→176, qs_base +16→+48, new qh_base, nib5 qh promotion) — Q6_K coopmat fusion explicitly deferred (16 signed scales/no-dmin not a clean drop-in for the M3.6 cache). **MEASURED real NVIDIA: AT-2814/2819 BIT-EXACT (max_diff==0); combined 2.47e-5 (Q6_K) / 2.80e-5 (Q5_K_M) / ≤1.7e-6 (coopmat, K=256/512/14336) ≤ FROZEN 1e-3.** Pessimistic code review independently re-derived BOTH decodes vs canonical ggml (incl. get_scale_min_k4 j<4/j≥4 byte indexing) — co-derived-layout risk retired. 1 design-revision cycle (full-256 golden, AT-2823 f32accum re-gate, builtin-⊆-baseline wording, oracle pointer). AT-2811..2823. |
...
IMPLEMENTED as M3.14 — see the status-snapshot row. CPU oracles + portable kernels for both formats, Q5_K_M coopmat variant on the M3.6 leader pattern, zero compiler changes, bit-exact on real NVIDIA. Q6_K coopmat fusion deferred (layout not a clean drop-in for the M3.6 scale cache). AT-2811..2823.
...
| ~~M3.19 low warts~~ ✅ FIXED (M3.23): IntLiteralOutOfRange at all 5 parser sites (negated-i64::MIN exact); CRLF boundary fix; 512-byte note cap | — | — |
...
| ~~M3.22 PW-1~~ ✅ FIXED (M3.23): shared-len hole names added to referenced_holes | — | — |
| Pre-existing axc-runtime defect (discovered M3.23, root-caused): workgroup-SHARED-memory kernels dispatched via the RESIDENT harness crash certain Lavapipe builds (SIGSEGV/heap corruption; Mesa 25.2.8) — NVIDIA unaffected; recommend CI Mesa-version pin + runtime investigation | medium | investigation |
| Pre-existing: axc-runtime at_1579 prints PASS then hangs at process exit (observed during M3.23 sweeps; a stuck instance predates the branch) | low | investigation |
| M3.23 reviewer note: the ordinal-remap skip branch (VariantsDiverge naming under earlier-variant-compile-failure) deserves a GPU-free unit test with synthetic result lists | low | 50 LOC |
