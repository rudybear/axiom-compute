...
| TFLOPS (GpuTimestamp MIN, kernel-only) | 10.907 | 101.88 |
...
- **SAME-SHAPE headline ratio (AXIOM fused GEMM / llama same-shape, basis `same_shape_gemm m=4096,n=512,k=14336`): 0.10706** (llama 9.34x faster)
- Cross-shape CONTEXT (NOT the kill criterion): AXIOM fused GEMM 10.907 TFLOPS vs llama n=1 GEMV 7.42 TFLOPS (ratio 1.470) — labeled cross-shape, never the headline.
- FLOP-consistency: ok=True (recomputed 101.883 vs reported 101.880 TFLOPS)
- **Kill-criterion (DESIGN §5, within 15% on NVIDIA): FAIL**
- Qualifier: M3.5 SAME-SHAPE fused GEMM vs llama's same-shape Q4_K MUL_MAT (101.88 TFLOPS). AXIOM measured 10.91 TFLOPS = ~9x BEHIND on throughput — a MASSIVE improvement from M3.4's ~87,000x cross-shape matvec. DESIGN §5 kill-criterion is 'within 15% on ANY vendor'; this does NOT fire (AXIOM behind). AMD/Intel pending hardware (EB.1).
- Fairness caveat: Same machine, same ICD, kernel-only-vs-kernel-only, SAME SHAPE (m=4096,n=512,k=14336 both sides), FLOP convention identical (2*m*n*k matmul MACs, dequant excluded). AXIOM's fused kernel is numerically VALID at this A/B shape (max_rel_diff=2.05e-06 <= frozen 1e-3); the ratio is a usable-kernel comparison. Ratio reported AS MEASURED for a numerically-VALID result at the A/B K. The n=1 GEMV (llama 7.42 TFLOPS) is cross-shape CONTEXT ONLY — never the headline (the SAME llama kernel runs 13.7x faster at n=512), so headlining AXIOM-GEMM vs llama-n1-GEMV would flatter AXIOM ~13x; that is forbidden (CRITICAL-1).
