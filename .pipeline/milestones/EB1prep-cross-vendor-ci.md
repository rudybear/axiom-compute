...
- **YAML schema validity** of all 3 new workflow files + `ci.yml` — `actionlint` (job-level, understands the Actions schema, catches bad `runs-on`, unknown keys, expression errors — supply-chain-pinned per §6.4, invoked with no path args so it auto-discovers every file under `.github/workflows` regardless of `.yml`/`.yaml` extension) and an in-test **`saphyr`** parse (§6.3, unconditional — no silent no-op, glob over `*.{yml,yaml}`).
- **Structural / security invariants** by parsing the YAML: activation-var `if` present on every vendor job; same-repo fork guard present on **every self-hosted job in every workflow file** (incl. `pytorch-interop`); `pull_request_target` absent everywhere; `permissions: contents: read`; `timeout-minutes` present on every self-hosted job; A/B + bless jobs are `workflow_dispatch`-only; bench-gate job never sets `AXC_BLESS_BASELINES: "1"`; every job invoking the `--test bench_regression` command sets `AXC_ENABLE_BENCH_REGRESSION: "1"` in its effective env (AT-3036).
...
## 8. Acceptance tests (AT-3019 .. AT-3037) — all CPU-only, run in normal CI
...
| **AT-3036** | `eb1_workflow_ci.rs` | **(review fix, optimistic must-fix)** Every job across **all** workflow files whose steps invoke `--test bench_regression` has `AXC_ENABLE_BENCH_REGRESSION: "1"` in its effective env (job-level or step-level) — glob-scanned, not hardcoded to `amd-gpu`/`intel-gpu`, so a future job that runs the bench_regression test inherits the same no-vacuous-gate guarantee. |
| **AT-3037** | `eb1_workflow_ci.rs` | **(review fix, PESS-1)** `all_workflow_files()`'s accepted extension set is exactly `{yml, yaml}` — pins the fix for the `.yaml`-extension security-scan bypass demonstrated at code review. |
