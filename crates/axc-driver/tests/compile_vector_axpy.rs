...
//!   AT-325-h: in-process spirv-tools validation (always mandatory, no PATH dependency)
...
fn validate_spirv(words: &[u32], label: &str) {
    ...
    validator.validate(words, None)
        .unwrap_or_else(|e| panic!("AT-325-h: spirv-tools rejected {label} SPIR-V: {e}"));
}
...
fn test_compile_vector_axpy_produces_valid_spirv() {
    ...
    // AT-325-h: in-process SPIR-V validation via spirv-tools crate (always mandatory).
    validate_spirv(&words, "vector_axpy");
